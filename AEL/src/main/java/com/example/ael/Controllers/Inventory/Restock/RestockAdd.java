...
public class RestockAdd {
    ...
    public void initialize() {
        date_purchaseDate.setValue(LocalDate.now());
        refreshCombo();
    }
    ...
    private boolean saveRecord() {
        ...
        String addQuery = "INSERT INTO Restocks (ProductID, Quantity, TotalCost, PurchaseDate, Supplier) VALUES ("
                + (prodID + ", ")
                + (Integer.parseInt(quantity) + ", ")
                + (Double.parseDouble(totalCost) + ", ")
                + ("'" + purchaseDate + "', ");
        if(supplier.equals("")) { addQuery+= ("null"); } else { addQuery+= ("'" + supplier + "' "); }
        addQuery += (");");
