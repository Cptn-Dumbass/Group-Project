...
public class OrdersAdd {
    ...
    //todo probably adjust a lot of the method names in this to more accurate ones
    @FXML
    ...
    private void onAddressIDChanged() {
        if(combo_AddrID.getSelectionModel().getSelectedItem().equals(newText)){
            ...
            populateAddressFields(Integer.parseInt(combo_AddrID.getSelectionModel().getSelectedItem().toString()));
    ...
    private void onCustomerIDChanged() {
        if(combo_CustID.getSelectionModel().getSelectedItem().equals(newText)){
            ...
            populateCustomerFields(Integer.parseInt(combo_CustID.getSelectionModel().getSelectedItem().toString()));
    ...
    private void populateFields() {
        ...
        //todo select print in combo, lock other fields accordingly
        //receiveddate = today?

        text_RefNo.setText(reference);
    }
    ...
    private boolean saveRecord(){
        ...
        if(status.equals("Approved") || status.equals("Sent")) {
            ...
            postageQuery = "INSERT INTO Postage(ReferenceNum, Weight, Size, Class, Service, TrackingNumber) VALUES ("
                    + (reference + "', ")
                    + (pkgWeight + ", ")
                    //+ ("'"+size+"',")
                    //+ ("'"+class+"',")
        ...
        String ordersQuery =   "INSERT INTO Orders(ReferenceNum, ReceivedDate, Source, ExternalReference, ChannelReference, PaymentMethod, Stage, PurchasePrice, CustomerID, AddressID, ProcessedDate) VALUES ("
                            + ("'" + reference + "', ")
                            + ("'" + receivedDate + "', ")
                            + ("'" + source + "', ")
                            + ("'" + extRef + "', ")
        ...
        try {
            ...
            if(addrResults.next()){
                ...
                autoAddrID = Integer.parseInt(addrID);
            }
            ...
            int autoCustID = 0;
            if(custResults.next()) {
                autoCustID = custResults.getInt(1);
            }
            else {
                autoCustID = Integer.parseInt(custID);
            }
    ...
    private void populatePostageFields(String status) { //todo strip editable/disable logic from this and delete the rest
    ...
    private void populateAddressFields(Integer addressID) { //this needs to stay for population from existing ids
        Connection connection = DatabaseManager.connect();
    ...
    private String generateNewRef() {
        String newestRefQuery = "SELECT ReferenceNum FROM Orders ORDER BY ReferenceNum DESC LIMIT 1;";

        Connection connection = DatabaseManager.connect();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(newestRefQuery);
            ResultSet resultSet = preparedStatement.executeQuery();
            resultSet.next();

            Long refLong = (Long.parseLong(resultSet.getString("ReferenceNum").replace("-", "")));
            refLong++;
            String newRefStripped = (refLong).toString();

            newRefStripped = new StringBuilder(newRefStripped).insert(3, "-").toString();
            String newRef = new StringBuilder(newRefStripped).insert(11, "-").toString();

            //todo need to check that this doesnt exist in the database
            return newRef;
        }
        catch (SQLException e) {
            new Alert(Alert.AlertType.ERROR, "Error generating new Reference Number, please enter manually.").showAndWait(); return null;
        } finally {
            DatabaseManager.closeConnection(connection);
        }
    }
